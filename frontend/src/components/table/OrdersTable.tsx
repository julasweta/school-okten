...
const OrdersTable: React.FC = () => {
  ...
  const filter = {
    ...
    userId:
      searchQuery.userId === "select"
        ? undefined
        : searchQuery.userId || userIdParam || undefined,
    ...
    course:
      searchQuery.course === "select"
        ? undefined
        : searchQuery.course || courseParam || undefined,
    course_format:
      searchQuery.course_format === "select"
        ? undefined
        : searchQuery.course_format || course_formatParam || undefined,
    course_type:
      searchQuery.course_type === "select"
        ? undefined
        : searchQuery.course_type || course_typeParam || undefined,
    status:
      searchQuery.status === "select"
        ? undefined
        : searchQuery.status || statusParam || undefined,
    groupName:
      searchQuery.groupName === "select" || searchQuery.groupName === undefined
        ? undefined
        : searchQuery.groupName || groupNameParam || undefined,
    limit: limit,
  };
    ...
    try {
      dispatch(
        ordersActions.getOrders({
          ...
          course_format:
            filter.course_format === undefined ? "" : filter.course_format,
          course_type:
            filter.course_type === undefined ? "" : filter.course_type,
            ...
            orders.map((order) => (
              <React.Fragment key={order._id}>
                ...
                >
                  ...
                  <td>{order.status}</td>
                  <td>{order.sum}</td>
                  <td>{order.already_paid ? "Yes" : "No"}</td>
